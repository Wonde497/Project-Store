...
interface IRFCardReader {
	 ...
	 * </ul>
	 * @param responseData - the response plain from the card
	 ...
	 * @param plain - 读取返回的块数据(16字节数据) | the plain from the block, length: 16
	 * @return 成功0x00，其他失败 | 0 means success, others while failure
     * \_en_
     * @brief read a block
     *
	 * @param blockNo the block No.
	 * @param plain the plain from the block, length: 16
	 * @return 0 means success, others while failure
     * \en_e
     * \code{.java}
     * \endcode
	 ...
	 * @param blockNo - 块号 | the block No.
	 * @param plain - 指定数据写到指定的块里 | the source plain
	 * @return 成功0x00，其他失败 | 0 means success, others while failure
     * \_en_
     * @brief write a block
     *
