 ...
 * Use the {@link deletesupp#newInstance} factory method to
 * create an instance of this fragment.
 */
public class deletesupp extends Fragment {
       ...
       // userList= user_fun.viewUsers(selection,selectionArgs,user_type);
        ...
        for(int i = 0; i < rows; i ++) {
            ...
            if (i == -1) {
                ...
                if(row.getTpe()=="support"){
                tv.setText(row.getName());
                tv.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
                tv.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                       // user_fun.deleterow(tv.getText().toString());
                        Toast.makeText(view_frag.getContext(),"deleted",Toast.LENGTH_SHORT).show();
                        //tv2.setText("DISABLED");
                    }
                });}else {
                    Toast.makeText(view_frag.getContext(),"no support registerd",Toast.LENGTH_SHORT).show();
